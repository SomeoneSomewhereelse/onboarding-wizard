...
## A real Stripe API key, inherited from old bot-project audit docs, was committed into this repo's git history and caught by GitHub's push protection before the first push
...
- **What happened:** The restructure's baseline commit (created to bring this repo's ~305 untracked files under version control for the first time — see the restructure's own design/plan docs) included several old `bot`-project audit/findings documents (`docs/2026-08-05-first-hosted-run-findings.md`, `docs/2026-08-11-full-project-review-security-performance-quality.md`, `docs/superpowers/plans/2026-08-11-audit-fix-round.md`, `docs/superpowers/specs/2026-08-11-audit-fix-round-design.md`) that, unrelated to anything this project actually uses Stripe for, contained a real Stripe API key. The check performed before that commit was filename-based only (scanning the staged file list for `.env`/`*.pem`/known secret filenames) — no content-level secret scan was run across the ~305 files being committed for the first time. A later task in the same restructure deleted those specific documents from the working tree (unrelated docs-pruning cleanup, not aware of the secret), but that deletion did not remove the key from git *history* — the blob remained reachable via the earlier commit, and `git push` would have published it. GitHub's server-side push protection caught it and rejected the push before anything reached the remote.
- **Cost:** None — the key never left this machine; GitHub's push protection is exactly the backstop that worked here. But it's a real gap: this project's own secret-handling discipline (never commit `.env`, never touch secret-bearing files carelessly) was written with *known* secret-shaped files (`.env`, `*.pem`, service-account JSON) in mind, not "an old prose document that happens to quote a credential inline" — and a bulk first-commit of hundreds of pre-existing files is exactly the situation where that gap matters most.
- **Fix:** Rewrote local git history with `git filter-repo --replace-text` (via `uv tool run git-filter-repo`, since the repo had never been pushed anywhere yet, making history rewrite safe/low-cost) using structural regex patterns for Stripe's known key-prefix shapes (`sk_live_`/`sk_test_`/`rk_live_`/`rk_test_`/`pk_live_`/`pk_test_`/`whsec_`) — the actual key value was never read, viewed, or printed at any point; the fix was a pattern-based substitution, verified afterward by confirming zero matches for the same patterns across `git log -p --all`. Force-pushed the rewritten history, then deleted the local backup tag and ran `git gc --prune=now --aggressive` so the original value doesn't persist in local objects either. Recommended the user rotate the key regardless, since a flagged live-looking credential should be treated as compromised the moment it's found, independent of whether it actually left the machine.
- **Suggested CLAUDE.md change:** A bulk first-commit of a large pre-existing tree (bringing previously-untracked files under version control, or importing a directory from elsewhere) needs a content-level secret scan — not just a filename-based one — before that commit is created, not just before every push thereafter. A filename check (`.env`, `*.pem`, known service-account JSON names) only catches secrets that live in files whose *names* signal them; a credential pasted inline into a prose document (an incident writeup, an audit finding, a chat log export) has no such signal and will sail through a filename-only check. GitHub's push protection is a real backstop, but relying on it as the *only* check means a secret can still sit in local history (and any other clone/fork made before a push) even when it never reaches the remote.
